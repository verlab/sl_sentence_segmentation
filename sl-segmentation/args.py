...
flags.DEFINE_integer('steps_per_epoch', 750, 'Number of batches per epoch')
flags.DEFINE_float('learning_rate', 0.001, 'Learning rate')
flags.DEFINE_integer('stop_patience', 100, 'Patience for early stopping')
flags.DEFINE_integer('batch_size', 8, 'Batch size for training')
flags.DEFINE_integer('test_batch_size', 1, 'Batch size for evaluation')

# Model flags
flags.DEFINE_float('class_weight', 5.0, 'Weight of class 1')
flags.DEFINE_float('input_dropout', 0.0, 'Input dropout rate')
flags.DEFINE_bool('include_pose', False, 'Include the pose as input with optical flow?')
flags.DEFINE_integer('encoder_layers', 5, 'Number of RNN layers')
flags.DEFINE_bool('encoder_bidirectional', False,
                  'Use a bidirectional encoder?')
flags.DEFINE_integer('hidden_size', 64, 'RNN hidden state size')
